...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ error: "ID do processo ausente na URL" }, { status: 400 });
    }
    ...
    if (equipamentos.length === 0) {
      return NextResponse.json({ error: "Nenhum equipamento encontrado para gerar relatório." }, { status: 404 });
    }
    ...
    setores.forEach(s => columns.push({ header: s, key: s, width: 6 }));
    ...
    itens.forEach((item, index) => {
      item.item = index + 1;
      item.solicitarQtde = { formula: `${item.total} - ${item.saldoAtual}`, result: item.total }; 
    ...
    worksheet.eachRow((row, rowNumber) => {
      if (rowNumber > 1) { 
        row.eachCell((cell, colNumber) => {
          ...
          if (colNumber > 3 && colNumber <= (3 + setores.length) && cell.value === 0) {
            cell.value = '';
          }
        });
      }
